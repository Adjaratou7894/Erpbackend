...
public interface ListePostulantRepository extends JpaRepository<Liste_postulant, Long> {
    ...
    @Query(value = "SELECT postulant.id,postulant.nom_postulant,postulant.prenom_postulant,postulant.email,postulant.genre from postulant,liste_postulant WHERE liste_postulant.idliste=postulant.liste_postulant_idliste AND liste_postulant.libelleliste=:lib", nativeQuery = true)
    List<Object> trouvertout(String lib);
